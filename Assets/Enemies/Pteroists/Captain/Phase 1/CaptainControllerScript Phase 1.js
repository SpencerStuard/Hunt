...
var numSummoned:int;
//Brawlers
var brawlerSummon:GameObject;
var brawlerDropL:GameObject;
var brawlerDropR:GameObject;
//Blowers
var blowerSummon:GameObject;
var blowerDropL:GameObject;
var blowerDropR:GameObject;
//Turrets
var turretSummon:GameObject;
var turretDropL:GameObject;
var turretDropR:GameObject;
//Escape
var escapePtero:GameObject;
...
function Update () 
{
	...
	SummonEnemies();		//Summon enemies to fight player.
...
function OnGUI()		//Sets the frame, background, and adjusts the current HP bar to the enemies current HP.
{
	if(Options.hpBars)
	{
	    GUI.DrawTexture(new Rect(screenPosition.x, screenPosition.y-195, 100, 20), background, ScaleMode.StretchToFill);
	    GUI.DrawTexture(new Rect(screenPosition.x, screenPosition.y-195, 100*enemyHP/maxHP, 20), foreground, ScaleMode.StretchToFill);
	    GUI.DrawTexture(new Rect(screenPosition.x-2, screenPosition.y-196, 103, 23), frame, ScaleMode.StretchToFill);
    }
}
*/
...
function SummonEnemies()
{
	if(numSummoned<6)
	{
		if(brawlerSummon==null&&!summoning)
			if(Random.Range(0,2)==0)
			{
				summoning=true;
				anim.SetTrigger("Summon");
				brawlerSummon=Instantiate(brawlerDropL,Vector3(transform.position.x+10,transform.position.y+5,0),Quaternion.identity);
				brawlerSummon.transform.localScale.x=-1;
				numSummoned++;
			...
			else
			{
				summoning=true;
				anim.SetTrigger("Summon");
				brawlerSummon=Instantiate(brawlerDropR,Vector3(transform.position.x-10,transform.position.y+5,0),Quaternion.identity);
				brawlerSummon.transform.localScale.x=1;
				numSummoned++;
				yield WaitForSeconds(3);
				summoning=false;
			}
		if(turretSummon==null&&!summoning)
			if(Random.Range(0,2)==0)
			{
				summoning=true;
				anim.SetTrigger("Summon");
				turretSummon=Instantiate(turretDropL,Vector3(transform.position.x-10,transform.position.y+5,0),Quaternion.identity);
				numSummoned++;
				yield WaitForSeconds(3);
				summoning=false;
			}
			else
			{
				summoning=true;
				anim.SetTrigger("Summon");
				turretSummon=Instantiate(turretDropR,Vector3(transform.position.x+10,transform.position.y+5,0),Quaternion.identity);
				numSummoned++;
				yield WaitForSeconds(3);
				summoning=false;
			}
		if(blowerSummon==null&&!summoning)
			if(Random.Range(0,2)==0)
			{
				summoning=true;
				anim.SetTrigger("Summon");
				blowerSummon=Instantiate(blowerDropL,Vector3(transform.position.x-10,transform.position.y+5,0),Quaternion.identity);
				blowerSummon.transform.localScale.x=1;
				numSummoned++;
				yield WaitForSeconds(3);
				summoning=false;
			}
			else
			{
				summoning=true;
				anim.SetTrigger("Summon");
				blowerSummon=Instantiate(blowerDropR,Vector3(transform.position.x+10,transform.position.y+5,0),Quaternion.identity);
				blowerSummon.transform.localScale.x=-1;
				numSummoned++;
				yield WaitForSeconds(3);
				summoning=false;
	...
	else if(numSummoned>=6&&blowerSummon==null&&turretSummon==null&&brawlerSummon==null&&!escaping)
		Escape();
}

function Escape()		//If all summons are dead, summon an escape Ptero
{
	escaping=true;
	var escapePteroClone=Instantiate(escapePtero,Vector3(transform.position.x-10,transform.position.y+5,0),Quaternion.identity);
	escapePteroClone.GetComponent(MonoBehaviour).captain=gameObject;
}
